<template>
  <div :class="['sidebar', expanded ? 'expanded' : 'no-expanded']">
    ...
    $route.path === '/login' ? 'activeDescription' : ''
      ...
      <div class="description" :class="[
    expanded ? 'show-description' : '',
    $route.path === '/settings' ? 'activeDescription' : ''
  ]">
        Ustawienia
      </div>
    </router-link>
    <hr>
<!-- ============================= HOME ============================== -->
    <router-link to="/" title="" class="menu-item"
...
export default {
  setup() {
    ...
    return { router, route, expanded, toggleExpanded, handleItemClick };
  },
};
</script>
